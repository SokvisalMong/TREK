...
const additionalCategoryIdsSchema = z
  .array(z.number().int().positive())
  .max(500)
  .describe('Complete additional category ID set; duplicates and the primary category are removed');
...
export function registerPlaceTools(server: McpServer, userId: number, scopes: string[] | null): void {
      ...
      'update_place',
      {
        ...
        inputSchema: {
          ...
          category_id: z
            ...
            .nullable()
            .optional()
            .describe('Primary category ID, or null to clear it'),
          additional_category_ids: additionalCategoryIdsSchema
            .optional()
            .describe('Replacement additional category set; omit to preserve it, [] to clear it'),
  ...
  if (W)
    server.registerTool(
      ...
      async ({ tripId, placeId }) => {
        ...
        } catch {} // sync journeys before the row is gone
  ...
  if (R)
    server.registerTool(
      ...
      {
        description:
          'List all places/POIs in a trip. Category filters match the primary or any additional category using OR semantics.',
        inputSchema: {
          tripId: z.number().int().positive(),
          ...
          category: z
            ...
            .optional()
            .describe(
              'Legacy single category ID, or "uncategorized" for places with no primary or additional categories',
            ),
          category_ids: additionalCategoryIdsSchema.optional().describe('Category IDs matched with OR semantics'),
      ...
      async ({ tripId, search, category, category_ids, tag, assignment }) => {
        if (!canAccessTrip(tripId, userId)) return noAccess();
        const places = listPlaces(String(tripId), { search, category, category_ids, tag, assignment });
        return ok({ places });
      },
    );
  ...
  if (W)
    server.registerTool(
      ...
      {
        description:
          'Update many places in a trip with the same values. category_id controls the primary visual category; additional_category_ids replaces the complete additional set (omit to preserve, [] to clear).',
        inputSchema: {
          tripId: z.number().int().positive(),
          placeIds: z
            .array(z.number().int().positive())
