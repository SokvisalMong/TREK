...
import type { Tag, Participant, Category } from '../../types';
...
export class QueryHelpersService {
  ...
  /** Batch-load additional shared categories for multiple trip places. */
  loadAdditionalCategoriesByPlaceIds(placeIds: number[]): Record<number, Category[]> {
    const categoriesByPlaceId: Record<number, Category[]> = {};
    for (const placeId of placeIds) categoriesByPlaceId[placeId] = [];
    if (placeIds.length === 0) return categoriesByPlaceId;

    const rows = this.db.all<Category & { place_id: number }>(
      `SELECT pac.place_id, c.id, c.name, c.color, c.icon
       FROM place_additional_categories pac
       JOIN categories c ON c.id = pac.category_id
       WHERE pac.place_id IN (${placeIds.map(() => '?').join(',')})
       ORDER BY c.name ASC, c.id ASC`,
      ...placeIds,
    );
    for (const { place_id: placeId, ...category } of rows) categoriesByPlaceId[placeId].push(category);
