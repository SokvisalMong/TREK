...
export class CollectionsService {
  ...
  private hydratePlaces(rows: PlaceRow[]): CollectionPlace[] {
    ...
    const additionalCategoriesByPlace = loadAdditionalCategoriesByCollectionPlaceIds(this.db.connection, ids);
    return rows.map(r => {
      ...
      return {
        ...
        category: r.category_id
          ...
          : undefined,
        additional_category_ids: (additionalCategoriesByPlace[r.id] || []).map(({ id }) => id),
        additional_categories: additionalCategoriesByPlace[r.id] || [],
  ...
  savePlace(userId: number, body: CollectionSavePlaceRequest, socketId?: string): CollectionSaveResult {
    ...
    const categoryWrite = normalizePlaceCategoryWrite(this.db.connection, body, null);
  ...
  `,
        ...
        categoryWrite.categoryId, body.price ?? null, body.currency ?? null, body.notes ?? null,
      ...
      replaceCollectionPlaceAdditionalCategories(
        this.db.connection,
        id,
        categoryWrite.additionalCategoryIds ?? [],
      );
  ...
  saveFromTripPlace(
    ...
    const additionalCategoryIds = (loadAdditionalCategoriesByPlaceIds(this.db.connection, [placeId])[placeId] ?? []).map(
      ({ id }) => id,
    );
  ...
  saveFromTripPlaces(
    ...
    const additionalCategoriesBySource = loadAdditionalCategoriesByPlaceIds(this.db.connection, placeIds);
  ...
  `);
    ...
    this.db.transaction(() => {
      for (const placeId of placeIds) {
        ...
        const copiedId = Number(res.lastInsertRowid);
        replaceCollectionPlaceAdditionalCategories(
          this.db.connection,
          copiedId,
          (additionalCategoriesBySource[placeId] ?? []).map(({ id }) => id),
        );
        copied++;
      }
    });
  ...
  async updatePlace(userId: number, placeId: number, body: CollectionPlaceUpdateRequest, socketId?: string): Promise<CollectionPlace> {
    ...
    const existingCategory = this.db.get<{ category_id: number | null }>(
      'SELECT category_id FROM collection_places WHERE id = ?',
      placeId,
    );
    const categoryWrite = normalizePlaceCategoryWrite(
      this.db.connection,
      body,
      existingCategory?.category_id ?? null,
    );
  ...
  copyToTrip(userId: number, body: CollectionCopyToTripRequest): { copied: number; skipped: { id: number; name: string }[] } {
    ...
    const additionalCategoriesBySource = loadAdditionalCategoriesByCollectionPlaceIds(
      this.db.connection,
      sources.map(({ id }) => id),
    );
  ...
  `);
    ...
    const insertAdditional = this.db.prepare(
      'INSERT OR IGNORE INTO place_additional_categories (place_id, category_id) VALUES (?, ?)',
    );
    ...
    this.db.transaction(() => {
      for (const s of sources) {
        ...
        for (const category of additionalCategoriesBySource[s.id] ?? []) {
          insertAdditional.run(newPlaceId, category.id);
        }
