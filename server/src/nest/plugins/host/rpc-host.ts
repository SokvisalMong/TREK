...
export interface HostDeps {
  ...
  /** A trip's place pool, hydrated like the REST list with primary/additional categories and tags. */
  listTripPlaces(tripId: number): unknown[];
...
export class PluginRpcHost {
  ...
  constructor(
    ...
    if (has('db:read:trips')) {
      ...
      // The trip's place pool is hydrated through the same service as REST so
      // plugin reads preserve tags and both primary/additional category fields.
      this.methods.set('trips.getPlaces', (p, uid) =>
        this.tripRead(p, uid, () => deps.listTripPlaces(num(p.tripId, 'tripId'))),
      );
