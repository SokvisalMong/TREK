...
export interface PlaceCreateInput {
  ...
  category_id?: number | null; additional_category_ids?: number[]; price?: number; currency?: string;
...
export class PlacesService {
  ...
  list(
    tripId: string,
    filters: { search?: string; category?: string; category_ids?: number[]; tag?: string; assignment?: 'all' | 'unassigned' | 'assigned' },
  ) {
    let query = `
  ...
  `;
    ...
    if (filters.category_ids?.length) {
      ...
      query += ` AND (p.category_id = ? OR EXISTS (
        SELECT 1 FROM place_additional_categories pac
        WHERE pac.place_id = p.id AND pac.category_id = ?
      ))`;
    ...
    const additionalCategoriesByPlaceId = loadAdditionalCategoriesByPlaceIds(this.dbs.connection, placeIds);
    ...
    return places.map(p => ({
      ...
      additional_category_ids: (additionalCategoriesByPlaceId[p.id] || []).map(({ id }) => id),
      additional_categories: additionalCategoriesByPlaceId[p.id] || [],
  ...
  create(tripId: string, body: PlaceCreateInput) {
    const {
      name, description, lat, lng, address, price, currency,
      place_time, end_time,
    ...
    const categoryWrite = normalizePlaceCategoryWrite(this.dbs.connection, body, null);

    const placeId = this.dbs.transaction(() => {
  ...
  `,
        ...
        categoryWrite.categoryId, price ?? null, currency || null,
      ...
      replacePlaceAdditionalCategories(this.dbs.connection, id, categoryWrite.additionalCategoryIds ?? []);

      if (tags.length > 0) {
        const insertTag = this.dbs.prepare('INSERT OR IGNORE INTO place_tags (place_id, tag_id) VALUES (?, ?)');
        for (const tagId of this.tagsOnTrip(tripId, tags)) insertTag.run(id, tagId);
      }
      return id;
    });
  ...
  async update(
    ...
    const { result, reclaim } = this.dbs.transaction(() => this.applyUpdate(tripId, placeId, body, ifMatch));
  ...
  private applyUpdate(
    ...
    const categoryWrite = normalizePlaceCategoryWrite(this.dbs.connection, body, existingPlace.category_id ?? null);
    const {
      name, description, lat, lng, address, price, currency,
      place_time, end_time,
      duration_minutes, notes, image_url, google_place_id, google_ftid, osm_id, website, phone,
  ...
  `,
    ...
    if (categoryWrite.additionalCategoryIdsProvided) {
      ...
      this.dbs.run(
        'DELETE FROM place_additional_categories WHERE place_id = ? AND category_id = ?',
        placeId,
        categoryWrite.categoryId,
      );
    }
