...
import { additionalCategoryIdsSchema, placeWebsiteSchema } from '@trek/shared'
import { z } from 'zod';
...
export class PlacesMcp {
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      additional_category_ids: additionalCategoryIdsSchema.optional().describe('Additional category IDs; the primary category is removed automatically'),
  ...
  async createPlace(
    { tripId, name, description, lat, lng, address, category_id, additional_category_ids, google_place_id, google_ftid, osm_id, notes, website, phone, price, currency }: {
      tripId: number; name: string; description?: string; lat?: number; lng?: number; address?: string;
      category_id?: number; additional_category_ids?: number[]; google_place_id?: string; google_ftid?: string; osm_id?: string;
    ...
    const place = this.places.create(String(tripId), { name, description, lat, lng, address, category_id, additional_category_ids, google_place_id, google_ftid, osm_id, notes, website, phone, price, currency });
  ...
  async createAndAssignPlace(
    { tripId, dayId, name, description, lat, lng, address, category_id, additional_category_ids, google_place_id, google_ftid, osm_id, place_notes, website, phone, assignment_notes, price, currency }: {
    ...
    try {
      const result = this.db.transaction(() => {
        const place = this.places.create(String(tripId), { name, description, lat, lng, address, category_id, additional_category_ids, google_place_id, google_ftid, osm_id, notes: place_notes, website, phone, price, currency });
  ...
  async updatePlace(
    { tripId, placeId, name, description, lat, lng, address, category_id, additional_category_ids, price, currency, place_time, end_time, duration_minutes, notes, website, phone, transport_mode, osm_id, google_place_id, google_ftid }: {
      tripId: number; placeId: number; name?: string; description?: string; lat?: number; lng?: number;
      address?: string; category_id?: number; additional_category_ids?: number[]; price?: number; currency?: string; place_time?: string;
    ...
    if (!this.db.canAccessTrip(tripId, ctx.userId)) return noAccess();
    if (!this.guards.hasTripPermission('place_edit', tripId, ctx.userId)) return permissionDenied();
    const place = await this.places.update(String(tripId), String(placeId), { name, description, lat, lng, address, category_id, additional_category_ids, price, currency, place_time, end_time, duration_minutes, notes, website, phone, transport_mode, osm_id, google_place_id, google_ftid });
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      category_ids: additionalCategoryIdsSchema.optional().describe('Category IDs; matches primary or additional categories with OR semantics'),
  ...
  async listPlaces(
    { tripId, search, category, category_ids, tag, assignment }: {
      tripId: number; search?: string; category?: string; category_ids?: number[]; tag?: string;
      assignment?: 'all' | 'unassigned' | 'assigned';
    },
    ctx: McpContext,
  ) {
    if (!this.db.canAccessTrip(tripId, ctx.userId)) return noAccess();
    const places = this.places.list(String(tripId), { search, category, category_ids, tag, assignment });
    return ok({ places });
  }
  ...
  @Tool({
    ...
    inputSchema: {
      ...
      category_id: z.number().int().positive().optional().describe('Category ID — use list_categories'),
      additional_category_ids: additionalCategoryIdsSchema.optional().describe('Replacement additional category IDs'),
      price: z.number().optional(),
      currency: z.string().length(3).optional(),
      transport_mode: z.enum(['walking', 'driving', 'cycling', 'transit', 'flight']).optional(),
      place_time: z.string().max(50).optional().describe('Scheduled time (e.g. "09:00")'),
  ...
  async bulkUpdatePlaces(
    { tripId, placeIds, category_id, additional_category_ids, price, currency, transport_mode, place_time, end_time, duration_minutes, notes, website, phone, description }: {
      tripId: number; placeIds: number[]; category_id?: number; additional_category_ids?: number[]; price?: number; currency?: string;
    ...
    const fields = { category_id, additional_category_ids, price, currency, transport_mode, place_time, end_time, duration_minutes, notes, website, phone, description };
