...
function parseCategoryIds(value: string | string[] | undefined): number[] | undefined {
...
export class PlacesController {
  ...
  list(
    ...
    return {
      places: this.places.list(tripId, {
        search,
        category,
        category_ids: parseCategoryIds(categoryIds),
        tag,
      }),
    };
  }
  ...
  async bulkUpdate(
    ...
    // Key presence — not truthiness — distinguishes clearing a field from
    // omitting it. Either category field is a valid bulk update.
    const hasPrimary = Object.prototype.hasOwnProperty.call(body, 'category_id');
    const hasAdditional = Object.prototype.hasOwnProperty.call(body, 'additional_category_ids');
    if (!hasPrimary && !hasAdditional) {
      throw new HttpException({ error: 'Provide at least one field to update' }, 400);
    }
    const patch = {
      ...(hasPrimary ? { category_id: body.category_id } : {}),
      ...(hasAdditional ? { additional_category_ids: body.additional_category_ids } : {}),
    };
    const updated = await this.places.updateMany(tripId, ids, patch);
