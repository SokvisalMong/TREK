  ...
  getSharedTripData(token: string): Record<string, any> | null {
    ...
    if (permissions.share_map) {
      ...
      if (dayIds.length > 0) {
        ...
        const additionalCategoriesByPlace = this.queryHelpers.loadAdditionalCategoriesByPlaceIds(placeIds);
        ...
        for (const a of allAssignments as any[]) {
          ...
          byDay[a.day_id].push({
            ...
            place: {
              ...
              additional_category_ids: (additionalCategoriesByPlace[a.place_id] ?? []).map(({ id }) => id),
              additional_categories: additionalCategoriesByPlace[a.place_id] ?? [],
      ...
      const tripPlaces = this.dbs.all<Record<string, unknown> & { id: number; image_url: string | null }>(`
        SELECT p.*, c.name as category_name, c.color as category_color, c.icon as category_icon
        FROM places p LEFT JOIN categories c ON p.category_id = c.id
        WHERE p.trip_id = ? ORDER BY p.created_at DESC
      `, tripId);
      const additionalCategoriesByPlace = this.queryHelpers.loadAdditionalCategoriesByPlaceIds(
        tripPlaces.map((p) => p.id),
      );
      places = tripPlaces.map((p) => ({
        ...p,
        image_url: rewritePlacePhotoUrl(p.image_url, token),
        additional_category_ids: (additionalCategoriesByPlace[p.id] ?? []).map(({ id }) => id),
        additional_categories: additionalCategoriesByPlace[p.id] ?? [],
      }));
    }
