...
export class DaysService {
  ...
  `, dayId);
    ...
    const additionalCategoriesByPlaceId = this.queryHelpers.loadAdditionalCategoriesByPlaceIds([
      ...new Set(assignments.map((a) => a.place_id)),
    ]);
    ...
    return assignments.map(a => {
      ...
      return {
        ...
        place: {
          ...
          additional_category_ids: (additionalCategoriesByPlaceId[a.place_id] || []).map(({ id }) => id),
          additional_categories: additionalCategoriesByPlaceId[a.place_id] || [],
          tags,
        }
      };
    });
  ...
  `, ...dayIds);
    ...
    const additionalCategoriesByPlaceId = this.queryHelpers.loadAdditionalCategoriesByPlaceIds(placeIds);
    ...
    for (const a of allAssignments) {
      ...
      assignmentsByDayId[a.day_id].push(
        formatAssignmentWithPlace(
          a,
          tagsByPlaceId[a.place_id] || [],
          participantsByAssignment[a.id] || [],
          additionalCategoriesByPlaceId[a.place_id] || [],
        ),
      );
    }
