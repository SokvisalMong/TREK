...
import {
  loadAdditionalCategoriesByCollectionPlaceIds,
  loadAdditionalCategoriesByPlaceIds,
  normalizePlaceCategoryWrite,
  replaceCollectionPlaceAdditionalCategories,
...
export function roleOf(userId: number, collectionId: number): EffectiveRole {
  ...
  const row = db
    .prepare("SELECT role FROM collection_members WHERE collection_id = ? AND user_id = ? AND status = 'accepted'")
...
function ownerOf(collectionId: number): number {
  const row = db.prepare('SELECT owner_id FROM collections WHERE id = ?').get(collectionId) as
    | { owner_id: number }
    | undefined;
...
interface PlaceRow extends Omit<CollectionPlace, 'additional_categories' | 'additional_category_ids' | 'links'> {
...
function loadTagsByCollectionPlaceIds(
  placeIds: number[],
...
function hydratePlaces(rows: PlaceRow[]): CollectionPlace[] {
  ...
  const additionalCategoriesByPlace = loadAdditionalCategoriesByCollectionPlaceIds(db, ids);
  return rows.map((row) => {
    const { category_name, category_color, category_icon, ...rest } = row;
    const additionalCategories = additionalCategoriesByPlace[row.id];
    return {
      ...rest,
      ...
      tags: tagsByPlace[row.id] || [],
      label_ids: labelsByPlace[row.id] || [],
    };
  });
}
...
function collectionIdOfPlace(placeId: number): number {
  const row = db.prepare('SELECT collection_id FROM collection_places WHERE id = ?').get(placeId) as
...
function getCollectionRow(id: number): Collection {
  const col = db.prepare('SELECT * FROM collections WHERE id = ?').get(id) as
    | (Collection & { links?: unknown })
    | undefined;
  if (!col) httpError(404, 'Collection not found');
  const placeCount = (
    db.prepare('SELECT COUNT(*) AS n FROM collection_places WHERE collection_id = ?').get(id) as { n: number }
  ).n;
...
export function createCollection(userId: number, body: CollectionCreateRequest): Collection {
  const max = (
    db.prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM collections WHERE owner_id = ?').get(userId) as {
      m: number;
    }
  ).m;
  const result = db
...
export function updateCollection(
  ...
  if (body.cover_image !== undefined) {
    ...
    params.push(body.cover_image ?? null);
  }
  if (body.links !== undefined) {
    updates.push('links = ?');
    params.push(serializeLinks(body.links));
  }
...
export function setCollectionCover(userId: number, id: number, coverUrl: string | null, socketId?: string): Collection {
  ...
  const prev =
    (
      db.prepare('SELECT cover_image FROM collections WHERE id = ?').get(id) as
...
export function deleteCollection(userId: number, id: number): void {
  ...
  const accepted = (
    db.prepare("SELECT user_id FROM collection_members WHERE collection_id = ? AND status = 'accepted'").all(id) as {
      user_id: number;
    }[]
  ).map((r) => r.user_id);
  const pending = (
    db.prepare("SELECT user_id FROM collection_members WHERE collection_id = ? AND status = 'pending'").all(id) as {
      user_id: number;
    }[]
  ).map((r) => r.user_id);
...
function buildDedupSet(collectionId: number): DedupSet {
  const rows = db
    .prepare('SELECT name, lat, lng FROM collection_places WHERE collection_id = ?')
...
function findDuplicateCollectionPlace(
  ...
  if (candidate.lat != null && candidate.lng != null) {
    ...
    `,
        ...
        .get(collectionId, candidate.lat, COORD_DEDUP_TOLERANCE, candidate.lng, COORD_DEDUP_TOLERANCE) as
...
export function savePlace(userId: number, body: CollectionSavePlaceRequest, socketId?: string): CollectionSaveResult {
  ...
  if (!body.force) {
    const duplicate = findDuplicateCollectionPlace(body.collection_id, {
      name: body.name,
      lat: body.lat,
      lng: body.lng,
    });
    if (duplicate) return { duplicate: true, duplicateOf: duplicate };
  }

  const ownerId = ownerOf(body.collection_id);
  const placeId = db.transaction(() => {
    ...
    const result = db
      ...
      .run(
        ...
        body.phone ?? null,
        body.status ?? 'idea',
        body.source_trip_id ?? null,
        body.source_place_id ?? null,
        serializeLinks(body.links),
      );

    const newPlaceId = Number(result.lastInsertRowid);
    replaceCollectionPlaceAdditionalCategories(db, newPlaceId, categoryWrite.additionalCategoryIds ?? []);
    attachTags(newPlaceId, body.tag_ids);
    return newPlaceId;
  })();
...
export function saveFromTripPlaces(
  ...
  assertCanEdit(userId, collectionId);
  if (!canAccessTrip(tripId, userId)) httpError(404, 'Trip not found');
  if (placeIds.length === 0) return { copied: 0, skipped: [] };

  const placeholders = placeIds.map(() => '?').join(',');
  const sourceRows = db
    .prepare(`SELECT * FROM places WHERE trip_id = ? AND id IN (${placeholders})`)
    .all(tripId, ...placeIds) as Place[];
  const sourceById = new Map(sourceRows.map((place) => [place.id, place]));
  const additionalCategoriesByPlaceId = loadAdditionalCategoriesByPlaceIds(
    db,
  ...
  db.transaction(() => {
    for (const sourcePlaceId of placeIds) {
      const place = sourceById.get(sourcePlaceId);
      if (!place) continue;
      const lat = place.lat ?? null;
      const lng = place.lng ?? null;
      if (!force && findDuplicateCollectionPlace(collectionId, { name: place.name, lat, lng })) {
        skipped.push({ id: sourcePlaceId, name: place.name });
        continue;
      }
      ...
      const newPlaceId = Number(result.lastInsertRowid);
      const additionalCategoryIds = additionalCategoriesByPlaceId[sourcePlaceId]
        .map(({ id }) => id)
        .filter((id) => id !== place.category_id);
      replaceCollectionPlaceAdditionalCategories(db, newPlaceId, additionalCategoryIds);
      copied++;
    }
  })();
...
export function updatePlace(
  ...
  const current = db.prepare('SELECT collection_id, category_id FROM collection_places WHERE id = ?').get(placeId) as
    | { collection_id: number; category_id: number | null }
    | undefined;
  if (!current) httpError(404, 'Place not found');
  const currentCollection = current.collection_id;
  ...
  db.transaction(() => {
    const categoryWrite = normalizePlaceCategoryWrite(db, body, current.category_id);
    const updates: string[] = [];
    const params: (string | number | null)[] = [];
    if (body.name !== undefined) {
      updates.push('name = ?');
    ...
    if (categoryWrite.categoryIdProvided) {
      updates.push('category_id = ?');
      params.push(categoryWrite.categoryId);
    }
    if (body.links !== undefined) {
      updates.push('links = ?');
      params.push(serializeLinks(body.links));
    ...
    if (movedTo !== null) {
      ...
      params.push(ownerOf(movedTo));
    }

    const relationChanged =
      categoryWrite.additionalCategoryIdsProvided ||
      body.tag_ids !== undefined ||
      body.label_ids !== undefined ||
      movedTo !== null;
    if (updates.length > 0 || relationChanged) {
      updates.push('updated_at = CURRENT_TIMESTAMP');
      params.push(placeId);
    ...
    if (categoryWrite.additionalCategoryIdsProvided) {
      replaceCollectionPlaceAdditionalCategories(db, placeId, categoryWrite.additionalCategoryIds ?? []);
    } else if (categoryWrite.categoryIdProvided && categoryWrite.categoryId !== null) {
      db.prepare(
        'DELETE FROM collection_place_additional_categories WHERE collection_place_id = ? AND category_id = ?',
      ).run(placeId, categoryWrite.categoryId);
    }
    ...
    if (movedTo !== null) db.prepare('DELETE FROM collection_place_labels WHERE collection_place_id = ?').run(placeId);
  ...
  if (movedTo !== null) notifyCollectionUsers(movedTo, socketId, 'collections:updated');
...
export function setStatus(
  ...
  assertCanEdit(userId, collectionId);
  db.prepare('UPDATE collection_places SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(
    status,
    placeId,
  );
...
export function copyToTrip(
  ...
  const roleRow = db.prepare('SELECT role FROM users WHERE id = ?').get(userId) as { role: string } | undefined;
  ...
  for (const placeId of body.place_ids) {
    const row = db
      .prepare(
      ...
      .get(placeId) as (typeof sources)[number] | undefined;
    if (!row) httpError(404, 'Place not found');
    assertAccess(userId, row.collection_id);
    sources.push(row);
  }

  const additionalCategoriesBySourceId = loadAdditionalCategoriesByCollectionPlaceIds(
    db,
    sources.map(({ id }) => id),
  );
  // Trip dedup set (mirrors placeService.buildDedupSet).
  const existing = db.prepare('SELECT name, lat, lng FROM places WHERE trip_id = ?').all(body.trip_id) as Array<{
    name: string | null;
    lat: number | null;
    lng: number | null;
  }>;
  ...
  for (const row of existing) {
    if (row.name) dedup.names.add(row.name.trim().toLowerCase());
    else if (row.lat != null && row.lng != null) dedup.coords.push({ lat: row.lat, lng: row.lng });
  }
  ...
  db.transaction(() => {
    for (const source of sources) {
      if (!body.force && isCollectionPlaceDuplicate({ name: source.name, lat: source.lat, lng: source.lng }, dedup)) {
        skipped.push({ id: source.id, name: source.name });
        continue;
      }
      ...
      const newPlaceId = Number(result.lastInsertRowid);
      const additionalCategoryIds = additionalCategoriesBySourceId[source.id]
        .map(({ id }) => id)
        .filter((id) => id !== source.category_id);
      replacePlaceAdditionalCategories(db, newPlaceId, additionalCategoryIds);

      const tagIds = db
        .prepare('SELECT tag_id FROM collection_place_tags WHERE collection_place_id = ?')
        .all(source.id) as Array<{ tag_id: number }>;
      for (const { tag_id: tagId } of tagIds) insertTag.run(newPlaceId, tagId);

      if (source.name) dedup.names.add(source.name.trim().toLowerCase());
      else if (source.lat != null && source.lng != null) dedup.coords.push({ lat: source.lat, lng: source.lng });
      copied++;
    }
  })();
...
export function findMembership(
  ...
  if (query.google_place_id) {
    conditions.push('cp.google_place_id = ?');
    params.push(query.google_place_id);
  }
  ...
  return {
    saved: rows.length > 0,
    lists: rows.map((r) => ({ collection_id: r.collection_id, name: r.name, place_id: r.place_id })),
  };
}
...
export function notifyCollectionUsers(
  collectionId: number,
  excludeSid: string | undefined,
  event = 'collections:updated',
): void {
  const owner = db.prepare('SELECT owner_id FROM collections WHERE id = ?').get(collectionId) as
    | { owner_id: number }
    | undefined;
  if (!owner) return;
  const userIds = [owner.owner_id];
  const members = db
    .prepare("SELECT user_id FROM collection_members WHERE collection_id = ? AND status = 'accepted'")
    .all(collectionId) as { user_id: number }[];
...
function collectionIdOfLabel(labelId: number): number {
  const row = db.prepare('SELECT collection_id FROM collection_labels WHERE id = ?').get(labelId) as
    | { collection_id: number }
    | undefined;
...
function getLabelById(labelId: number): CollectionLabel {
  return db
    .prepare('SELECT id, collection_id, name, color, sort_order FROM collection_labels WHERE id = ?')
    .get(labelId) as CollectionLabel;
}
...
export function createLabel(
  ...
  const count = (
    db.prepare('SELECT COUNT(*) AS n FROM collection_labels WHERE collection_id = ?').get(collectionId) as { n: number }
  ).n;
  if (count >= MAX_LABELS_PER_COLLECTION) httpError(400, `A list can have at most ${MAX_LABELS_PER_COLLECTION} labels`);
  if (
    db
      .prepare('SELECT 1 FROM collection_labels WHERE collection_id = ? AND lower(name) = lower(?)')
      .get(collectionId, trimmed)
  ) {
  ...
  const nextSort =
    (
      db
        .prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM collection_labels WHERE collection_id = ?')
        .get(collectionId) as { m: number }
    ).m + 1;
  const res = db
    .prepare('INSERT INTO collection_labels (collection_id, name, color, sort_order) VALUES (?, ?, ?, ?)')
...
export function updateLabel(
  ...
  if (body.name !== undefined) {
    ...
    if (
      db
        .prepare('SELECT 1 FROM collection_labels WHERE collection_id = ? AND lower(name) = lower(?) AND id != ?')
        .get(collectionId, trimmed, labelId)
    ) {
      httpError(409, 'A label with this name already exists');
    }
    updates.push('name = ?');
...
export function sendInvite(
  ...
  const existing = db
    .prepare('SELECT id, status FROM collection_members WHERE collection_id = ? AND user_id = ?')
    .get(collectionId, targetUserId) as { id: number; status: string } | undefined;
  if (existing) {
  ...
  db.prepare("INSERT INTO collection_members (collection_id, user_id, status, role) VALUES (?, ?, 'pending', ?)").run(
    collectionId,
    targetUserId,
    role,
  );
...
export function acceptInvite(
  ...
  const invite = db
    .prepare("SELECT id FROM collection_members WHERE collection_id = ? AND user_id = ? AND status = 'pending'")
...
export function declineInvite(userId: number, collectionId: number, socketId: string | undefined): void {
  db.prepare("DELETE FROM collection_members WHERE collection_id = ? AND user_id = ? AND status = 'pending'").run(
    collectionId,
    userId,
  );
...
export function leaveCollection(userId: number, collectionId: number, socketId: string | undefined): void {
  ...
  db.prepare("DELETE FROM collection_members WHERE collection_id = ? AND user_id = ? AND status = 'accepted'").run(
    collectionId,
    userId,
  );
...
export function removeMember(ownerId: number, collectionId: number, targetUserId: number): void {
  ...
  const res = db
    .prepare("DELETE FROM collection_members WHERE collection_id = ? AND user_id = ? AND status = 'accepted'")
    .run(collectionId, targetUserId);
...
export function setMemberRole(
  ...
  if (!isOwner(ownerId, collectionId)) httpError(403, 'Not allowed');
  const res = db
    .prepare("UPDATE collection_members SET role = ? WHERE collection_id = ? AND user_id = ? AND status = 'accepted'")
    .run(role, collectionId, targetUserId);
  if (res.changes === 0) httpError(404, 'Member not found');
...
export function findMembershipForUser(
  userId: number,
  collectionId: number,
): { is_member: boolean; is_owner: boolean; status: string | null } {
  if (isOwner(userId, collectionId)) return { is_member: true, is_owner: true, status: 'accepted' };
  const row = db
    .prepare('SELECT status FROM collection_members WHERE collection_id = ? AND user_id = ?')
    .get(collectionId, userId) as { status: string } | undefined;
