    ...
    db.transaction(() => {
      ...
      if (data.date_shift_mode === 'shift_all') {
        ...
        const newDateByDayId = new Map(
          (
            db.prepare('SELECT id, date FROM days WHERE trip_id = ?').all(tripId) as {
              id: number;
              date: string | null;
            }[]
          ).map((d) => [d.id, d.date]),
...
export function copyTripById(sourceTripId: string | number, newOwnerId: number, title?: string): number {
  ...
  const fn = db.transaction(() => {
    ...
    const oldAdditionalCategories = db
      .prepare(
        `SELECT pac.place_id, pac.category_id
         FROM place_additional_categories pac
         JOIN places p ON p.id = pac.place_id
         WHERE p.trip_id = ?`,
      )
      .all(sourceTripId) as Array<{ place_id: number; category_id: number }>;
    const insertAdditionalCategory = db.prepare(
      'INSERT OR IGNORE INTO place_additional_categories (place_id, category_id) VALUES (?, ?)',
    );
    for (const relation of oldAdditionalCategories) {
      const newPlaceId = placeMap.get(relation.place_id);
      if (newPlaceId) insertAdditionalCategory.run(newPlaceId, relation.category_id);
    }
