...
import type { Assignment } from '@trek/shared';
...
export function getAssignmentsForDay(dayId: number | string) {
  ...
  const placeIds = [...new Set(assignments.map(({ place_id }) => place_id))];
  const tagsByPlaceId = loadTagsByPlaceIds(placeIds, { compact: true });
  const participantsByAssignment = loadParticipantsByAssignmentIds(assignments.map(({ id }) => id));
  const additionalCategoriesByPlaceId = loadAdditionalCategoriesByPlaceIds(db, placeIds);

  return assignments.map((assignment) =>
    formatAssignmentWithPlace(
      assignment,
      tagsByPlaceId[assignment.place_id] || [],
      participantsByAssignment[assignment.id] || [],
      additionalCategoriesByPlaceId[assignment.place_id],
    ),
  );
}
...
export function listDays(tripId: string | number) {
  ...
  const assignmentsByDayId: Record<number, Assignment[]> = {};
  for (const assignment of allAssignments) {
    if (!assignmentsByDayId[assignment.day_id]) assignmentsByDayId[assignment.day_id] = [];
    assignmentsByDayId[assignment.day_id].push(
      formatAssignmentWithPlace(
        assignment,
        tagsByPlaceId[assignment.place_id] || [],
        participantsByAssignment[assignment.id] || [],
