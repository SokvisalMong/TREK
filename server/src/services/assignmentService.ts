...
export function getAssignmentWithPlace(assignmentId: number | bigint) {
  ...
  const tagsByPlaceId = loadTagsByPlaceIds([a.place_id]);
  const participantsByAssignment = loadParticipantsByAssignmentIds([a.id]);
  const additionalCategoriesByPlaceId = loadAdditionalCategoriesByPlaceIds(db, [a.place_id]);
