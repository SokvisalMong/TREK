...
describe('Trip members', () => {
  ...
  it('TRIP-GUEST-002 — guest CRUD is owner-only', async () => {
    ...
    expect((testDb.prepare('SELECT display_name FROM users WHERE id = ?').get(guestId) as any).display_name).toBe(
      'Junior',
    );
...
describe('Copy trip with data', () => {
  it('TRIP-026 — copy preserves days, places, tags, assignments, accommodations, reservations, budget, packing, notes', async () => {
    ...
    const additionalCategory = createCategory(testDb, { name: 'Architecture' });
    testDb
    ...
    const newAdditionalCategories = testDb
      .prepare(
      ...
      .all(newId);
    expect(newAdditionalCategories).toEqual([{ category_id: additionalCategory.id }]);
