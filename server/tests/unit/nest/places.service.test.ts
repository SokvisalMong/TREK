  ...
  const mock = {
    ...
    getPlaceWithTags: (placeId: any) => {
      ...
      return {
        ...
        additional_category_ids: additionalCategories.map((category: { id: number }) => category.id),
...
describe('list', () => {
  ...
  it('PLACE-SVC-006a — creates, filters, and updates additional categories', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    const primary = createCategory(testDb, { name: 'Primary', user_id: user.id });
    const extra = createCategory(testDb, { name: 'Extra', user_id: user.id });
    const replacement = createCategory(testDb, { name: 'Replacement', user_id: user.id });

    const place = svc.create(String(trip.id), {
      name: 'Multi-category place',
      category_id: primary.id,
      additional_category_ids: [extra.id, extra.id, primary.id],
    });
    expect(place.additional_category_ids).toEqual([extra.id]);
    expect(svc.list(String(trip.id), { category: String(extra.id) })).toHaveLength(1);

    const updated = await svc.update(String(trip.id), String(place.id), {
      additional_category_ids: [replacement.id, primary.id, replacement.id],
    }) as { additional_category_ids?: number[]; category_id?: number | null };
    expect(updated.additional_category_ids).toEqual([replacement.id]);

    await svc.update(String(trip.id), String(place.id), { category_id: replacement.id });
    const stored = testDb
      .prepare('SELECT category_id FROM place_additional_categories WHERE place_id = ?')
      .all(place.id) as Array<{ category_id: number }>;
