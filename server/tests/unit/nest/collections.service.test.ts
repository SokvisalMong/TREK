...
describe('saved places + dedup', () => {
  ...
  it('COLLECTIONS-SVC-013a: saves and updates additional categories', async () => {
    const u = createUser(testDb).user;
    const primary = createCategory(testDb, { name: 'Primary' });
    const extra = createCategory(testDb, { name: 'Extra' });
    const replacement = createCategory(testDb, { name: 'Replacement' });
    const col = svc.createCollection(u.id, { name: 'Multi-category' });

    const saved = svc.savePlace(u.id, {
      collection_id: col.id,
      name: 'Tagged place',
      category_id: primary.id,
      additional_category_ids: [extra.id, primary.id, extra.id],
    }).place!;
    expect(saved.additional_category_ids).toEqual([extra.id]);

    const updated = await svc.updatePlace(u.id, saved.id, {
