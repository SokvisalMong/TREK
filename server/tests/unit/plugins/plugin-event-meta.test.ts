...
describe('pluginEventMeta', () => {
  ...
  describe('snapshot (wave 9)', () => {
    ...
    it('includes normalized additional categories in place snapshots', () => {
      const place = pluginEventMeta('place:updated', {
        place: {
          id: 9,
          category_id: 2,
          additional_category_ids: [4, 7],
          additional_categories: [
            { id: 4, name: 'Cafe', color: '#f00', icon: 'coffee' },
            { id: 7, name: 'Nature', color: '#0f0', icon: 'trees' },
          ],
        },
      });
      expect(place?.snapshot).toMatchObject({
        id: 9,
        category_id: 2,
        additional_category_ids: [4, 7],
        additional_categories: [
