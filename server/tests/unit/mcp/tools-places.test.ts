...
describe('Tool: create_place', () => {
  ...
  it('creates and returns normalized additional categories', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
...
describe('Tool: update_place', () => {
  ...
  it('replaces or clears additional categories while omitted updates preserve them', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    const category = createCategory(testDb, { name: 'Cafe' });
    const place = createPlace(testDb, trip.id, { name: 'Cafe' });
    testDb
      .prepare('INSERT INTO place_additional_categories (place_id, category_id) VALUES (?, ?)')
      .run(place.id, category.id);

    await withHarness(user.id, async (h) => {
      const preserved = await h.client.callTool({
        name: 'update_place',
        arguments: { tripId: trip.id, placeId: place.id, notes: 'preserve' },
      });
      expect(parseToolResult(preserved)).toMatchObject({
        place: { additional_category_ids: [category.id] },
      });

      const cleared = await h.client.callTool({
        name: 'update_place',
        arguments: { tripId: trip.id, placeId: place.id, additional_category_ids: [] },
      });
      expect(parseToolResult(cleared)).toMatchObject({
...
describe('Tool: bulk_update_places', () => {
  ...
  it('bulk-replaces additional category sets', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    const category = createCategory(testDb, { name: 'Nature' });
    ...
    await withHarness(user.id, async (h) => {
      ...
      expect(parseToolResult(result)).toMatchObject({ count: 2 });
      expect(
        testDb.prepare('SELECT place_id, category_id FROM place_additional_categories ORDER BY place_id').all(),
      ).toEqual([
        { place_id: first.id, category_id: category.id },
        { place_id: second.id, category_id: category.id },
      ]);
    });
  });
});
...
describe('Tool: list_places', () => {
  ...
  it('filters by primary or additional category ids with OR semantics', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    const primary = createCategory(testDb, { name: 'Hotel' });
    const additional = createCategory(testDb, { name: 'Cafe' });
    const mixed = createPlace(testDb, trip.id, { name: 'Mixed', category_id: primary.id });
    const second = createPlace(testDb, trip.id, { name: 'Second' });
    testDb
      .prepare('INSERT INTO place_additional_categories (place_id, category_id) VALUES (?, ?), (?, ?)')
      .run(mixed.id, additional.id, second.id, additional.id);

    await withHarness(user.id, async (h) => {
      const result = await h.client.callTool({
        name: 'list_places',
        arguments: { tripId: trip.id, category_ids: [primary.id, additional.id] },
      });
      const data = parseToolResult(result);
      if (!data || typeof data !== 'object' || !('places' in data) || !Array.isArray(data.places)) {
        throw new Error('Expected a places array');
      }
      const returnedIds = data.places.flatMap((place) => {
        if (!place || typeof place !== 'object' || !('id' in place) || typeof place.id !== 'number') return [];
        return [place.id];
      });
      expect(returnedIds.sort()).toEqual([mixed.id, second.id].sort());
    });
  });
});
