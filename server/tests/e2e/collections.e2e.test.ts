...
describe('Collections e2e (real auth guard + real service + temp SQLite)', () => {
  ...
  it('COLLECTIONS-E2E-011: save a place (200) then copy it to a trip', async () => {
    ...
    const placed = db
      .prepare("SELECT reservation_status FROM places WHERE trip_id = ? AND name = 'Trevi Fountain'")
      .get(tripId) as { reservation_status: string };
    expect(placed.reservation_status).toBe('none'); // itinerary defaults
  });

  it('COLLECTIONS-E2E-013: multi-category place survives collection → trip → collection', async () => {
    const primary = createCategory(db as never, { name: 'Hotel E2E' });
    const promoted = createCategory(db as never, { name: 'Cafe E2E' });
    const retained = createCategory(db as never, { name: 'Nature E2E' });
    const sourceCollection = (
      await request(server)
        .post('/api/addons/collections')
        .set('Cookie', sessionCookie(ownerId))
        .send({ name: 'Multi source' })
    ).body;
    const saved = await request(server)
      .post('/api/addons/collections/places')
      .set('Cookie', sessionCookie(ownerId))
      .send({
        ...
        name: 'Multi-category lodge',
        category_id: primary.id,
        additional_category_ids: [promoted.id, retained.id, promoted.id, primary.id],
      });
    expect(saved.status).toBe(200);
    expect(saved.body.place.additional_category_ids.sort()).toEqual([promoted.id, retained.id].sort());

    const reloaded = await request(server)
      .get(`/api/addons/collections/${sourceCollection.id}`)
      .set('Cookie', sessionCookie(ownerId));
    expect(reloaded.body.places[0].additional_category_ids.sort()).toEqual([promoted.id, retained.id].sort());

    const patched = await request(server)
      .patch(`/api/addons/collections/places/${saved.body.place.id}`)
      .set('Cookie', sessionCookie(ownerId))
      .send({ category_id: promoted.id });
    expect(patched.status).toBe(200);
    expect(patched.body.category_id).toBe(promoted.id);
    expect(patched.body.additional_category_ids).toEqual([retained.id]);

    const copiedToTrip = await request(server)
      .post('/api/addons/collections/copy-to-trip')
      .set('Cookie', sessionCookie(ownerId))
      .send({ trip_id: tripId, place_ids: [saved.body.place.id] });
    expect(copiedToTrip.status).toBe(200);
    const tripPlace = db
      .prepare('SELECT id FROM places WHERE trip_id = ? AND name = ?')
      .get(tripId, 'Multi-category lodge') as { id: number };
    expect(
      db.prepare('SELECT category_id FROM place_additional_categories WHERE place_id = ?').all(tripPlace.id),
    ).toEqual([{ category_id: retained.id }]);

    const targetCollection = (
      await request(server)
        .post('/api/addons/collections')
        .set('Cookie', sessionCookie(ownerId))
        .send({ name: 'Multi target' })
    ).body;
    const copiedBack = await request(server)
      .post('/api/addons/collections/places/from-trip')
      .set('Cookie', sessionCookie(ownerId))
      .send({
        collection_id: targetCollection.id,
        source_trip_id: tripId,
        source_place_id: tripPlace.id,
      });
    expect(copiedBack.status).toBe(200);
    expect(copiedBack.body.place.additional_category_ids).toEqual([retained.id]);
  });
