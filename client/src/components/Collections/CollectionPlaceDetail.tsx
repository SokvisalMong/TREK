...
interface CollectionPlaceDetailProps {
  ...
  onSave: (patch: { name?: string; description?: string | null; links?: CollectionLink[]; category_id?: number | null; additional_category_ids?: number[]; label_ids?: number[]; image_url?: string | null; lat?: number | null; lng?: number | null; address?: string | null }) => Promise<void>
  ...
  const toggleAdditionalCategory = (nextCategoryId: number) => {
    ...
    setAdditionalCategoryIds(current =>
      current.includes(nextCategoryId)
        ? current.filter(id => id !== nextCategoryId)
          ...
          <div className="col-detail-edit">
            ...
            <div className="col-detail-field">
              ...
              <div className="col-detail-cats">
                <button type="button" onClick={() => selectPrimaryCategory(null)} className={categoryId == null ? 'col-detail-cat on' : 'col-detail-cat'}>{t('collections.noCategory')}</button>
                {categories.map(cat => {
                  const Icon = getCategoryIcon(cat.icon ?? undefined)
                  const on = categoryId === cat.id
                  return (
                    <button key={cat.id} type="button" onClick={() => selectPrimaryCategory(cat.id)} className={on ? 'col-detail-cat on' : 'col-detail-cat'} style={{ ['--cat' as string]: cat.color || '#6366f1' }}>
                ...
                {categories
                  ...
                  .map((cat) => {
                    ...
                    return (
                      <button
                        key={cat.id}
                        aria-label={`${cat.name} (${t('places.formAdditionalCategories')})`}
                        type="button"
              ...
              <div className="col-detail-field">
                ...
                <div className="col-detail-cats">
                  {labels.map(l => {
                    ...
                    return (
                      <button key={l.id} type="button" onClick={() => toggleLabel(l.id)} className={on ? 'col-detail-cat on' : 'col-detail-cat'} style={{ ['--cat' as string]: l.color || '#6366f1' }}>
