...
describe('CollectionPlaceDetail', () => {
  ...
  it('FE-COMP-COLDETAIL-044: renders, edits, and saves additional categories', async () => {
    const user = userEvent.setup();
    const additional: CollectionPlace = {
      ...place,
      additional_category_ids: [2],
      additional_categories: [CATEGORIES[1]],
    };
    const props = renderDetail({ place: additional, categories: CATEGORIES });

    await user.click(await screen.findByRole('button', { name: 'Edit' }));
    expect(screen.getAllByRole('button', { name: /Museums/ }).some((button) => button.classList.contains('on'))).toBe(
      true
    );
    await user.click(screen.getByRole('button', { name: /Save/i }));
    expect(props.onSave).toHaveBeenCalledWith(expect.objectContaining({ additional_category_ids: [2] }));
  });

  it('FE-COMP-COLDETAIL-045: applies the docked class for the desktop split anchor', async () => {
