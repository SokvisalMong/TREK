...
export function buildPlacePopupHtml(place: PlaceWithCategory, photoUrl: string | null): string {
  ...
  const category =
    place.category_name && place.category_icon
      ...
      : '';
  const additionalCategories = place.additional_categories?.length
    ? `<div style="font-size:10px;color:#6b7280;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${place.additional_categories
        .map(({ name }) => esc(name))
        .filter(Boolean)
        .join(' · ')}</div>`
    : '';
  const address = place.address ? `<div style="${ADDR_STYLE}">${esc(place.address)}</div>` : '';
  return `${CARD_OPEN}${img}<div style="${NAME_STYLE}">${esc(place.name)}</div>${category}${additionalCategories}${address}</div>`;
}
