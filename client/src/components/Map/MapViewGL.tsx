...
interface Props {
  ...
  onMapContextMenu?:
    | ((e: { latlng: { lat: number; lng: number }; originalEvent: MouseEvent | TouchEvent }) => void)
    | null;
...
export function MapViewGL({
  ...
  const [hoverPlace, setHoverPlace] = useState<
    (Place & { category_color?: string | null; category_icon?: string | null; category_name?: string | null }) | null
  >(null);
      ...
      if (!map.getSource(PLACE_CLUSTER_SOURCE_ID)) {
        ...
        const zoomToCluster = (e: any) => {
          const features =
            typeof map.queryRenderedFeatures === 'function'
              ? map.queryRenderedFeatures(e.point, {
                  layers: [PLACE_CLUSTER_CIRCLE_LAYER_ID, PLACE_CLUSTER_COUNT_LAYER_ID],
                })
              : [];
          ...
          const easeToZoom = (nextZoom: number) => {
            try {
              map.easeTo({ center: coordinates, zoom: nextZoom, duration: 350 });
            } catch {
              /* noop */
            }
          };
  ...
  useEffect(() => {
    ...
    const runReconcile = () => {
      raf = null;
      const features =
        map.querySourceFeatures(PLACE_CLUSTER_SOURCE_ID, { filter: ['!', ['has', 'point_count']] }) || [];
  ...
  const HoverIcon =
    (hoverPlace?.category_icon && CATEGORY_ICON_MAP[hoverPlace.category_icon]) || CATEGORY_ICON_MAP['MapPin'];
    ...
    <div className="relative h-full w-full">
      ...
      {!hoverDisabled && hoverPlace && hoverPos && !isMobile && (
        ...
        >
          ...
          {!!hoverPlace.additional_categories?.length && (
            <div style={{ fontSize: 10, color: '#6b7280', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {hoverPlace.additional_categories.map((category) => category.name).join(' · ')}
            </div>
          )}
          {hoverPlace.address && (
