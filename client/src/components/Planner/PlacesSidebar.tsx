...
const PlacesSidebar = React.memo(function PlacesSidebar(props: PlacesSidebarProps) {
  ...
  const {
    ...
    categories, selectedIds, exitSelectMode, onBulkChangeCategory, onBulkChangeAdditionalCategories, categoryPickerOpen, setCategoryPickerOpen,
    ...
    >
      ...
      {categoryPickerOpen && (
        <PlacesBulkCategoryModal
          ...
          onPickAdditional={(categoryIds) => {
            onBulkChangeAdditionalCategories?.(Array.from(selectedIds), categoryIds)
            setCategoryPickerOpen(false)
            exitSelectMode()
          }}
        />
