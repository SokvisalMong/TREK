import { matchesPlaceCategory } from '@trek/shared';
import {
  Calendar,
  Car,
  ChevronsDown,
...
export default function DayDetailPanel({
  ...
  const dayDetailPlugins = usePluginStore((s) => s.plugins).filter(
    (p) => p.type === 'widget' && p.slot === 'day-detail'
  );
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {editingTitle ? (
              ...
              <div
                className="text-content"
                style={{ fontSize: 13, fontWeight: 700, transition: 'font-size 0.15s ease' }}
              >
            ...
            {!collapsed && formattedDate && (
              <div
                ...
                style={{ fontSize: 'calc(12px * var(--fs-scale-body, 1))', marginTop: 1 }}
              >
                {formattedDate}
              </div>
            )}
              ...
              <div style={{ maxHeight: 250, overflowY: 'auto' }}>
                {(() => {
                  const filtered = hotelCategoryFilter
                    ? places.filter((place) => matchesPlaceCategory(place, hotelCategoryFilter))
                    : places;
