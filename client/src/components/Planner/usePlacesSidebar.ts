import { Bookmark, CalendarDays, ExternalLink, Navigation, Pencil, Trash2 } from 'lucide-react';
...
export interface PlacesSidebarProps {
  ...
  onBulkChangeAdditionalCategories?: (ids: number[], categoryIds: number[]) => void;
  days: Day[];
  isMobile: boolean;
...
export function usePlacesSidebar(props: PlacesSidebarProps) {
  const { tripId, places, assignments, selectedDayId, pushUndo, initialScrollTop, onScrollTopChange } = props;
  ...
  const filtered = useMemo(
    () =>
      places.filter((p) => {
        ...
        if (!matchesPlaceCategoryFilters(p, categoryFilters)) return false;
        if (
          search &&
