...
import { dayTotalCost, formatDate, formatMoneySum, formatTime, splitReservationDateTime } from '../../utils/formatters';
...
import PlaceCategoryBadges from '../shared/PlaceCategoryBadges';
...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const [hotelLegs, setHotelLegs] = useState<
    Record<number, { top?: { seg: RouteSegment; name: string }; bottom?: { seg: RouteSegment; name: string } }>
  >({});
  ...
  useEffect(() => {
    ...
    const planDay = (dayId: number) => {
      ...
      const bookends =
        day && optimizeFromAccommodation !== false ? getDayBookendHotels(day, days, accommodations) : null;
    ...
    (async () => {
      ...
      const hotelByDay: Record<
        number,
        { top?: { seg: RouteSegment; name: string }; bottom?: { seg: RouteSegment; name: string } }
      > = {};
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const selectionScrollKey =
    S.selectedAssignmentId != null
      ? `a${S.selectedAssignmentId}`
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {cost && (
                    <div style={{ marginTop: 2 }}>
                      <span className="text-[#059669]" style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))' }}>
                        {cost}
                      </span>
                    </div>
                  )}
                      ...
                      >
                        ...
                        {onPlanTransit ? (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onPlanTransit(day.id);
                            }}
                            title={t('transit.title')}
                            aria-label={t('transit.title')}
              ...
              {isExpanded && (
                ...
                >
                  {hotelLegs[day.id]?.top && (
                    <HotelRouteConnector
                      seg={hotelLegs[day.id]!.top!.seg}
                      name={hotelLegs[day.id]!.top!.name}
                  ...
                  {merged.length === 0 && !dayNoteUi ? (
                    ...
                    >
                      <span className="text-content-faint" style={{ fontSize: 'calc(12px * var(--fs-scale-body, 1))' }}>
                        {t('dayplan.emptyDay')}
                      </span>
                    </div>
                  ) : (
                      ...
                      if (item.type === 'place') {
                        ...
                        return (
                          <React.Fragment key={`place-${assignment.id}`}>
                              ...
                              onContextMenu={(e) => {
                                ...
                                ctxMenu.open(e, [
                                  ...
                                  collectionsEnabled && {
                                    ...
                                    icon: Bookmark,
                                    onClick: () => useSaveToCollectionStore.getState().open(placeToSaveTarget(place)),
                                  },
                                  { divider: true },
                                  canEditDays &&
                                    onDeletePlace && {
                                ...
                                {assignment.participants?.length > 0 && (
                                  <div style={{ marginTop: 3, display: 'flex', alignItems: 'center', gap: -4 }}>
                                    ...
                                    {assignment.participants.length > 5 && (
                                      <span
                                        className="text-content-faint"
                                        style={{ fontSize: 'calc(8px * var(--fs-scale-caption, 1))', marginLeft: 2 }}
                                      >
                                        +{assignment.participants.length - 5}
                                      </span>
                                    )}
                            ...
                            {routeLegs[day.id]?.[assignment.id] && (
                      ...
                      if (item.type === 'transport') {
                        ...
                        if (res.__leg) {
                          ...
                          const parts =
                            res.type === 'train'
                              ? [
                                  res.__leg.train_number,
                                  res.__leg.platform ? `Gl. ${res.__leg.platform}` : '',
                                  res.__leg.seat ? `Sitz ${res.__leg.seat}` : '',
                                ].filter(Boolean)
                        ...
                        const transitMeta =
                          res.type === 'transit' && meta.transit && Array.isArray(meta.transit.legs)
                            ? meta.transit
                            : null;
                        ...
                        return (
                          <React.Fragment key={`transport-${res.id}-${legKey}-${day.id}`}>
                            <div
                              ...
                              onDragStart={(e) => {
                                if (!canEditDays || spanPhase === 'middle' || res.__leg || dragDisabled) {
                                  e.preventDefault();
                                  return;
                                }
                              ...
                              style={{
                                ...
                                background: `${color}08`,
                                cursor: transitMeta || (canEditDays && onEditTransport) ? 'pointer' : 'default',
                                userSelect: 'none',
                  ...
                  {hotelLegs[day.id]?.bottom && (
                    <HotelRouteConnector
                      seg={hotelLegs[day.id]!.bottom!.seg}
                      name={hotelLegs[day.id]!.bottom!.name}
                      profile={routeProfile}
                      placement="bottom"
                    />
                  )}
                      ...
                      <div style={{ display: 'flex', gap: 6, alignItems: 'stretch' }}>
                        <button
                          onClick={() => {
                            ...
                            // so tapping Route on another day first points the selection here.
                              ...
                              onSelectDay(day.id, true);
                              if (!routeShown) onToggleRoute?.();
                            }
                          }}
                          className={
                            routeActive ? 'bg-accent text-accent-text' : 'bg-transparent text-content-secondary'
                          }
                          style={{
                            flex: 1,
                            display: 'flex',
                        ...
                        <button
                          onClick={() => {
                            ...
                            const dayStops = getDayAssignments(day.id).filter(
                              (a) => a.place?.lat != null && a.place?.lng != null
                            );
                            const stops = dayStops.map((a) => ({ lat: a.place!.lat!, lng: a.place!.lng! }));
                            const firstStop = dayStops[0]
                              ? { isPlace: true, time: dayStops[0].place?.place_time ?? null }
                              : undefined;
                            const lastAssignment = dayStops[dayStops.length - 1];
                            const lastStop = lastAssignment
                              ? { isPlace: true, time: lastAssignment.place?.place_time ?? null }
                              : undefined;
                            ...
                            const morning =
                              drawMorning &&
                              routeBookends?.morning?.place_lat != null &&
                              routeBookends?.morning?.place_lng != null
                                ? { lat: routeBookends.morning.place_lat, lng: routeBookends.morning.place_lng }
                                : null;
                            const evening =
                              drawEvening &&
                              routeBookends?.evening?.place_lat != null &&
                              routeBookends?.evening?.place_lng != null
                                ? { lat: routeBookends.evening.place_lat, lng: routeBookends.evening.place_lng }
                                : null;
