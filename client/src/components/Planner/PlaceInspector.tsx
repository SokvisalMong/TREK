...
export default function PlaceInspector({
  ...
  const placeDetailPlugins = usePluginStore((s) => s.plugins).filter(
    (p) => p.type === 'widget' && p.slot === 'place-detail'
  );
  ...
  useEffect(() => {
    if (placeIdForDetails == null) {
      setProviderDetails([]);
      return;
    }
    let cancelled = false;
    pluginsApi
      .placeDetails(placeIdForDetails)
      .then((d) => {
        if (!cancelled)
          setProviderDetails((d.providers || []).filter((p) => Array.isArray(p.items) && p.items.length > 0));
      })
      .catch(() => {
        if (!cancelled) setProviderDetails([]);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    if (!showSaveToCollection || !place) {
      setSavedInCollection(false);
      return;
    }
    let cancelled = false;
    collectionsApi
      ...
      .then((m) => {
        if (!cancelled) setSavedInCollection(m.saved);
      })
      .catch(() => {
        if (!cancelled) setSavedInCollection(false);
      });
    return () => {
      cancelled = true;
    };
        ...
        >
          ...
          <div className="hidden sm:flex" style={{ flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            ...
            {place.price > 0 && (
              <Chip
                ...
                text={formatMoney(Number(place.price) || 0, place.currency || tripCurrency || 'EUR', locale)}
                color="#059669"
                bg="#ecfdf5"
              />
            )}
          ...
          {mode === 'trip' && providerDetails.length > 0 && (
            <div
              ...
              style={{ borderRadius: 10, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}
            >
              {providerDetails.flatMap((p) =>
                p.items.map((it, i) => (
                  <div
                    key={`${p.pluginId}-${i}`}
                    style={{
                  ...
                  >
                    <span className="text-content-secondary" style={{ fontWeight: 500, flexShrink: 0 }}>
                      {it.label}
                    </span>
                    {it.url ? (
                      <a
                      ...
                      <span className="text-content-muted" style={{ textAlign: 'right' }}>
                        {it.value}
                      </span>
                    )}
                  </div>
              ...
              {placeDetailPlugins.map((p) => {
                ...
                return (
                  <div key={p.id} className="bg-surface-hover" style={{ borderRadius: 10, overflow: 'hidden' }}>
                    <PluginFrame
                      pluginId={p.id}
                      tripId={tid != null ? String(tid) : null}
            ...
            <ActionButton
              ...
              label={
                <span className="hidden sm:inline">
                  {savedInCollection ? t('inspector.savedToCollection') : t('inspector.saveToCollection')}
                </span>
              }
            />
          )}
            ...
            >
              ...
              >
                {member.avatar_url || member.avatar ? (
                  <img
                    src={member.avatar_url || avatarSrc(member.avatar)!}
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          {editingName ? (
            ...
            <span
              ...
              style={{
                ...
                fontSize: 'calc(15px * var(--fs-scale-subtitle, 1))',
                lineHeight: '1.3',
                cursor: onUpdatePlace ? 'text' : 'default',
              }}
            >
              {place.name}
            </span>
          ...
          <PlaceCategoryBadges categories={place.additional_categories} max={4} />
        </div>
        {place.address && (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{ fontSize: 'calc(10px * var(--fs-scale-caption, 1))', fontWeight: 700 }}
                      >
              ...
              {placeFiles.map((f) => (
                ...
                >
                  ...
                  {f.file_size && (
                    <span
                      className="text-content-faint"
                      style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', flexShrink: 0 }}
                    >
