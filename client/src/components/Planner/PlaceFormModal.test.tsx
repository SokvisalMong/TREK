...
describe('PlaceFormModal', () => {
  ...
  it('loads and submits existing additional categories while editing', async () => {
    const user = userEvent.setup();
    const primary = buildCategory({ name: 'Hotel' });
    ...
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<PlaceFormModal {...defaultProps} place={place} categories={[primary, cafe]} onSave={onSave} />);
  ...
  it('removes an additional category when it becomes primary', async () => {
    const user = userEvent.setup();
    ...
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<PlaceFormModal {...defaultProps} place={place} categories={[hotel, cafe]} onSave={onSave} />);
  ...
  it('keeps the additional category menu open and toggles a selected category', async () => {
    const user = userEvent.setup();
    const hotel = buildCategory({ name: 'Hotel' });
    const cafe = buildCategory({ name: 'Cafe' });
    const nature = buildCategory({ name: 'Nature' });
    const place = buildPlace({ name: 'Hotel cafe', category_id: hotel.id });
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<PlaceFormModal {...defaultProps} place={place} categories={[hotel, cafe, nature]} onSave={onSave} />);
