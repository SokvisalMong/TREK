...
function usePlaceFormModal(props: PlaceFormModalProps) {
  ...
  const handlePrimaryCategoryChange = (value: string | number) => {
    const categoryId = value === '' ? null : Number(value);
    setForm((prev) => ({
      ...prev,
      category_id: value === '' ? '' : String(value),
      additional_category_ids: prev.additional_category_ids.filter((id) => id !== categoryId),
    }));
  };

  const handleToggleAdditionalCategory = (value: string | number) => {
    const categoryId = Number(value);
    if (!Number.isInteger(categoryId) || categoryId <= 0) return;
    setForm((prev) => {
      if (String(categoryId) === prev.category_id) return prev;
      const additionalCategoryIds = prev.additional_category_ids.includes(categoryId)
        ? prev.additional_category_ids.filter((id) => id !== categoryId)
        : [...prev.additional_category_ids, categoryId];
      return { ...prev, additional_category_ids: additionalCategoryIds };
    });
  };
    ...
    try {
      ...
      if (cat) {
        setForm((prev) => ({
          ...
          additional_category_ids: prev.additional_category_ids.filter((id) => id !== cat.id),
        }));
      }
      setNewCategoryName('');
...
export default function PlaceFormModal(props: PlaceFormModalProps) {
  ...
  const selectedAdditionalCategories = categories.filter((category) =>
    form.additional_category_ids.includes(category.id)
  );
  const additionalCategoryOptions = categories
    .filter((category) => String(category.id) !== form.category_id)
    .map((category) => ({
      value: String(category.id),
      label: category.name,
      isSelected: form.additional_category_ids.includes(category.id),
    }));
  return (
    <Modal
      isOpen={isOpen}
    ...
    >
      <div className={twoColumn ? 'flex items-stretch gap-5' : ''}>
        <form
          onSubmit={handleSubmit}
          className={twoColumn ? 'min-w-0 flex-1 space-y-4' : 'space-y-4'}
          onPaste={handlePaste}
        >
          {/* Place Search */}
          ...
          {/* Additional shared categories classify and filter the place without
            changing its primary marker icon or color. */}
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">
              {t('places.formAdditionalCategories')}
            </label>
            <CustomSelect
              value=""
              onChange={handleToggleAdditionalCategory}
              placeholder={t('places.addAdditionalCategory')}
              options={additionalCategoryOptions}
              searchable
              keepOpenOnSelect
              disabled={additionalCategoryOptions.length === 0}
              size="sm"
            />
            {selectedAdditionalCategories.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1.5" aria-label={t('places.formAdditionalCategories')}>
                {selectedAdditionalCategories.map((category) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => handleRemoveAdditionalCategory(category.id)}
                      className="inline-flex rounded-full p-0.5 text-content-muted hover:text-content"
                      aria-label={t('places.removeAdditionalCategory', { name: category.name })}
                    >
        ...
        {twoColumn && <CollectionPicker bias={locationBias} onSelect={handleSelectMapsResult} t={t} />}
      </div>
    </Modal>
  );
}
