...
export function useTripPlanner() {
  ...
  const mapPlaces = useMemo(() => {
    ...
    return places.filter(p => {
      ...
      if (placesCategoryFilter.size > 0 && !matchesPlaceCategoryFilters(p, placesCategoryFilter)) return false
  ...
  const confirmChangeAdditionalCategories = useCallback(async (ids: number[], categoryIds: number[]) => {
    if (!ids.length) return
    const state = useTripStore.getState()
    const captured = state.places
      .filter((place) => ids.includes(place.id))
      .map((place) => ({ id: place.id, previous: place.additional_category_ids ?? [] }))
    try {
      ...
      if (captured.length > 0) {
        pushUndo(t('undo.changeCategory'), async () => {
          for (const place of captured) {
            await tripActions.updatePlacesMany(tripId, [place.id], {
              additional_category_ids: place.previous,
            })
          }
        })
      }
  ...
  return {
    ...
    handleSavePlace, openPlaceEditor, handleDeletePlace, confirmDeletePlace, confirmDeletePlaces, confirmChangeCategory, confirmChangeAdditionalCategories,
