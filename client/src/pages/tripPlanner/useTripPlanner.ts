...
} from '../../components/Planner/parsedItemToDraft';
import { resolvePluginIcon } from '../../components/shared/PluginIcon';
import { useToast } from '../../components/shared/Toast';
import { deleteImportFiles, getImportFiles, offlineDb } from '../../db/offlineDb';
...
import {
  toggleAllConnections as flipAllConnectionsMode,
  parseStoredConnections,
  resolveEffectiveConnections,
  resolveVisibleConnectionIds,
  toggleConnectionId,
  type StoredConnections,
} from '../../utils/connectionsVisibility';
import { matchesPlaceCategoryFilters } from '../../utils/placeCategories';
...
export function useTripPlanner() {
  ...
  const positioned = tripPagePlugins
    .filter((p) => p.tripPage?.position != null)
    .sort((a, b) => a.tripPage!.position! - b.tripPage!.position!);
  for (const p of positioned)
    TRIP_TABS.splice(Math.min(p.tripPage!.position!, TRIP_TABS.length), 0, {
      id: `plugin:${p.id}`,
      label: p.name,
      icon: resolvePluginIcon(p.icon),
    });
  for (const p of tripPagePlugins.filter((p) => p.tripPage?.position == null))
    TRIP_TABS.push({ id: `plugin:${p.id}`, label: p.name, icon: resolvePluginIcon(p.icon) });
  ...
  const [transitPrefill, setTransitPrefill] = useState<{
    from?: { name: string; lat: number; lng: number } | null;
    to?: { name: string; lat: number; lng: number } | null;
  } | null>(null);
  ...
  const mapPlaces = useMemo(() => {
    ...
    return places.filter((p) => {
      ...
      if (!matchesPlaceCategoryFilters(p, placesCategoryFilter)) return false;
      ...
      if (capturedPlace) {
        pushUndo(t('undo.deletePlace'), async () => {
          const newPlace = await tripActions.addPlace(tripId, {
            ...
            additional_category_ids: capturedPlace.additional_category_ids ?? [],
  ...
  const confirmChangeCategory = useCallback(
    async (ids: number[], categoryId: number | null) => {
      if (!ids.length) return;
      const state = useTripStore.getState();
      // Capture each place's prior category so undo can restore them per group.
      const captured = state.places
        .filter((p) => ids.includes(p.id))
        .map((p) => ({ id: p.id, prev: p.category_id ?? null }));
      try {
  ...
  const confirmChangeAdditionalCategories = useCallback(
    async (ids: number[], categoryIds: number[]) => {
      if (!ids.length) return;
      const state = useTripStore.getState();
      const captured = state.places
        ...
        .map((place) => ({
          id: place.id,
          previousCategoryIds: [...(place.additional_category_ids ?? [])].sort((left, right) => left - right),
        }));
      try {
        await tripActions.updatePlacesMany(tripId, ids, { additional_category_ids: categoryIds });
        toast.success(t('places.categoryChanged', { count: ids.length }));
        if (captured.length > 0) {
          pushUndo(t('undo.changeCategory'), async () => {
            const idsByPreviousSet: Record<string, number[]> = {};
            for (const { id, previousCategoryIds } of captured) {
              const key = previousCategoryIds.join(',');
              (idsByPreviousSet[key] ??= []).push(id);
            }
            for (const [key, group] of Object.entries(idsByPreviousSet)) {
              const previousCategoryIds = key ? key.split(',').map(Number) : [];
              await tripActions.updatePlacesMany(tripId, group, {
                additional_category_ids: previousCategoryIds,
              });
            }
          });
        }
    ...
    try {
      ...
      if (data.type === 'hotel' && acc && typeof acc.address === 'string') {
        ...
        if (address && linkedPlace && (linkedPlace.address || '') !== address) {
          try {
            await tripActions.updatePlace(tripId, linkedPlace.id, { address });
          } catch {
            /* keep saving the booking; the address still lands in location */
          }
        }
        delete acc.address;
      }
  ...
  const resolveImportedPlace = async (venue: { name?: string; address?: string | null }): Promise<number | null> => {
    ...
    if (n) {
      const existing =
        places.find((p) => p.name?.trim().toLowerCase() === n) ??
    ...
    try {
      ...
      /* geocode failure is non-fatal — create the place without coords */
    }
    try {
      const place = await placesApi.create(tripId, {
        name: name || address || 'Accommodation',
        lat,
        lng,
        address,
      } as never);
  ...
  const openImportItem = (item: BookingImportPreviewItem) => {
    ...
    if (isTransportItem(item)) {
      ...
      setEditingTransport(null);
      setTransportModalDayId(null);
      setTransportPrefill(draft);
      setShowTransportModal(true);
    } else {
      setShowTransportModal(false);
      setEditingTransport(null);
      setTransportPrefill(null);
      setTransportModalDayId(null);
      setEditingReservation(null);
      setReservationPrefill(draft);
      setShowReservationModal(true);
    }
  };
  ...
  return {
    ...
    confirmChangeAdditionalCategories,
    handleAssignToDay,
