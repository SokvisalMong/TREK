...
interface PlCategoryPickerProps {
  ...
  onAdditionalChange?: (categoryIds: number[]) => void
}
 ...
 */
export default function PlCategoryPicker({
  planner,
  value,
  onChange,
  additionalCategoryIds = [],
  onAdditionalChange,
}: PlCategoryPickerProps) {
  ...
  return (
    <div className="flex flex-wrap gap-[6px]">
      ...
      {onAdditionalChange && categories.some(cat => String(cat.id) !== value) && (
        <div className="mt-2 flex w-full flex-wrap gap-[6px]">
          <div className="w-full text-[0.65625rem] font-semibold uppercase tracking-[0.08em] text-m-muted">
            {t('places.formAdditionalCategories')}
          </div>
          {categories
            .filter(cat => String(cat.id) !== value)
            .map(cat => {
              const active = additionalCategoryIds.includes(cat.id)
              const Icon = getCategoryIcon(cat.icon)
              return (
                <button
                  key={`additional-${cat.id}`}
                  ...
                  onClick={() =>
                    onAdditionalChange(
                      active
                        ? additionalCategoryIds.filter(id => id !== cat.id)
                        : [...additionalCategoryIds, cat.id],
                    )
                  }
                  className={`${PILL_BASE} ${active ? 'bg-m-act text-m-actfg' : 'bg-[color:var(--m-ic)] text-m-muted'}`}
                >
