...
export const createPlacesSlice = (set: SetState, get: GetState): PlacesSlice => ({
  ...
  updatePlacesMany: async (tripId, placeIds, patch) => {
    if (placeIds.length === 0) return;
    try {
      const result = await placeRepo.updateMany(tripId, placeIds, patch as Record<string, unknown>);
      const idSet = new Set(placeIds);
      const normalizedById = new Map(result.places.map((place) => [place.id, place]));
      set((state) => {
        // Use the normalized server/offline response rather than the raw bulk
        // patch: each place may exclude a different primary-category collision.
        const updatedAssignments = { ...state.assignments };
        let changed = false;
        for (const [dayId, items] of Object.entries(state.assignments)) {
          if (items.some((assignment: Assignment) => assignment.place?.id != null && idSet.has(assignment.place.id))) {
            updatedAssignments[dayId] = items.map((assignment: Assignment) => {
              if (assignment.place?.id == null || !idSet.has(assignment.place.id)) return assignment;
              const normalized = normalizedById.get(assignment.place.id);
              return normalized
                ? {
                    ...assignment,
                    place: {
                      ...normalized,
                      place_time: assignment.place.place_time,
                      end_time: assignment.place.end_time,
                    },
                  }
                : assignment;
            });
        ...
        return {
          places: state.places.map((place) => normalizedById.get(place.id) ?? place),
          ...(changed ? { assignments: updatedAssignments } : {}),
        };
      });
    } catch (err: unknown) {
