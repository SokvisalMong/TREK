...
function mergePlacePatch<T extends { category_id?: number | null; additional_category_ids?: number[] }>(
  place: T,
  patch: Partial<Place>,
): T {
  const next = { ...place, ...patch } as T
  if (Object.prototype.hasOwnProperty.call(patch, 'additional_category_ids')) {
    next.additional_category_ids = (patch.additional_category_ids ?? []).filter((id) => id !== next.category_id)
  } else if (Object.prototype.hasOwnProperty.call(patch, 'category_id') && patch.category_id != null) {
    next.additional_category_ids = (place.additional_category_ids ?? []).filter((id) => id !== patch.category_id)
  }
  return next
}
    ...
    try {
      ...
      set(state => {
        ...
        for (const [dayId, items] of Object.entries(state.assignments)) {
          if (items.some((a: Assignment) => a.place?.id != null && idSet.has(a.place.id))) {
            updatedAssignments[dayId] = items.map((a: Assignment) =>
              a.place?.id != null && idSet.has(a.place.id)
                ? { ...a, place: mergePlacePatch(a.place, patch) }
                : a,
            )
            changed = true
          }
        ...
        return {
          places: state.places.map((p) => (idSet.has(p.id) ? mergePlacePatch(p, patch) : p)),
          ...(changed ? { assignments: updatedAssignments } : {}),
        }
      })
    } catch (err: unknown) {
