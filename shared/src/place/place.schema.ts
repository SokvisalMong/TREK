...
export const placeSchema = z.object({
  ...
  additional_category_ids: z.array(z.number()).optional(),
  additional_categories: z.array(placeCategoryItemSchema).optional(),
...
export const placeBulkUpdateRequestSchema = z
  .object({
    // Deliberately unbounded: an empty list is a valid no-op response.
    ids: z.array(z.number()),
    // null clears the primary category. Omitted fields preserve their values.
    category_id: placeCategoryIdSchema.nullable().optional(),
    // An empty array explicitly clears all additional categories.
    additional_category_ids: additionalCategoryIdsSchema.optional(),
  })
  .refine(
    ({ ids, category_id, additional_category_ids }) =>
      ids.length === 0 || category_id !== undefined || additional_category_ids !== undefined,
